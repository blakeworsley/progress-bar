...
const StackedBarGraph = ({ width, height, children, colors }) => {
  ...
  const add = (a, b) => a + b;
  const values = children.map(c => c.props.value); // children are anything inside the opening and closing tags
  const sum = values.reduce(add, 0); // 0 is so that you are starting with a 0
